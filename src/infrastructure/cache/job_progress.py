...
def _publish(meeting_id: str, mapping: dict):
    try:
        _redis.publish(f"job_updates:{meeting_id}", json.dumps(mapping))
    except Exception as e:
        print(f"[Warning] Failed to publish job update: {e}")
...
def set_job_queued(meeting_id: str) -> None:
    ...
    _redis.hset(_key(meeting_id), mapping=mapping)
    _redis.expire(_key(meeting_id), JOB_TTL)
    _publish(meeting_id, mapping)
...
def set_job_retrying(meeting_id: str, attempt: int, max_retries: int) -> None:
    """Mark the job as retrying."""
    mapping = {
        "status": "retrying",
        "stage": "retrying",
        "detail": f"Attempt {attempt} of {max_retries} failed. Retrying in 60s...",
        "progress": "0",
        "error": "",
    }
    _redis.hset(_key(meeting_id), mapping=mapping)
